...
class JY extends App {
	...
	public function testimonials() {
		...
		$data = $this->customPostsQuery($ids, ['text' => ['testimonial_quote','author_info','author_url'], 'attachments' => ['thumb']], 'posts.`post_date` DESC');
	...
	protected function hydrateTestimonial(&$post, $index, $featured_id) {
		...
		$post->author_info_parts = array_filter(explode("\n", $post->author_info));
